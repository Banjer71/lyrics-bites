...
const SongPage = (props) => {
  ...
  useEffect(() => {
    ...
    fetch(
      `https://cors-anywhere.herokuapp.com/https://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=${trackId}&apikey=${process.env.REACT_APP_API_KEY_MUSICMATCH}`
    )
  ...
  const getAlbumTracks = (idTrack, idAlbum) => {
    fetch(
      `https://cors-anywhere.herokuapp.com/https://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=${idTrack}&apikey=${process.env.REACT_APP_API_KEY_MUSICMATCH}`
    )
      .then((res) => res.json())
      .then((data) => {
        const lyric = data.message.body.lyrics;
        // console.log(lyric.lyrics_body.split(/\n\s/));
        let verse = lyric.lyrics_body.split(/\n\s/);
        console.log(verse);
